...
# Set up logging with detailed format for HF Spaces
logging.basicConfig(
    level=logging.INFO,
...
# CORS middleware - Dynamic origins from environment variable
# HF Spaces: Set ALLOWED_ORIGINS in Space settings (comma-separated)
ALLOWED_ORIGINS = os.getenv("ALLOWED_ORIGINS", "http://localhost:3000,https://masterpost-io.netlify.app").split(",")
...
# Directories (HF Spaces only allows /tmp to be writable)
...
# Mount static files for serving processed images (from /tmp)
app.mount("/processed", StaticFiles(directory="/tmp/processed"), name="processed")
...
if __name__ == "__main__":
    import uvicorn
    port = int(os.getenv("PORT", 7860))  # HF Spaces requires port 7860
    ...
    uvicorn.run(
        app,
        host="0.0.0.0",
        port=port,  # Use variable instead of reading env twice
        log_level="info",
        reload=False
    )